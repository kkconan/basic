...
public class PersistentCookieHttpSessionStrategy implements MultiHttpSessionStrategy, HttpSessionManager {
	...
	public void onNewSessionTwo(String sid, HttpServletRequest request, HttpServletResponse response,String terminal) {
		if (!StringUtil.isEmpty(TerminalConfig.loginScope) && TerminalConfig.loginScope.contains(terminal)) {
			Map<String, String> sessionIds = getSessionIds(request);
			String sessionAlias = getCurrentSessionAlias(request);
			sessionIds.put(sessionAlias, sid);
			Cookie sessionCookie = createSessionCookie(request, sessionIds);
	...
	public void onInvalidateSession(HttpServletRequest request, HttpServletResponse response) {
		if (request.getSession().getAttribute("UID") != null) {
			String userOid = (String) request.getSession().getAttribute("UID");
	 ...
	 * alias. If the proValue is null, then only a single session is supported per
	 * browser.
	 *
	...
	class MultiSessionHttpServletResponse extends HttpServletResponseWrapper {
		...
		public String encodeRedirectURL(String url) {
			...
			return PersistentCookieHttpSessionStrategy.this.encodeURL(url, getCurrentSessionAlias(request));
		}
		...
		public String encodeURL(String url) {
			...
			return PersistentCookieHttpSessionStrategy.this.encodeURL(url, alias);
		}
	}
